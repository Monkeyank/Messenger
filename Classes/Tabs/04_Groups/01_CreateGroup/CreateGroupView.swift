...
//-------------------------------------------------------------------------------------------------------------------------------------------------
class CreateGroupView: UIViewController, UISearchBarDelegate, UITableViewDataSource, UITableViewDelegate, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
	...
	func createGroup(name: String) {
		...
		if (Connection.isReachable()) {
			Group.createItem(name: name, picture: linkPicture, members: selection) { error in
				if (error == nil) {
					ProgressHUD.dismiss()
		...
		if let image = info[.editedImage] as? UIImage {
			...
			if let data = image.jpegData(compressionQuality: 0.6) {
				...
				UploadManager.upload(data: data, name: "group", ext: "jpg", progress: { progress in
					...
					if (error == nil) {
						self.imageGroup.image = squared
						DownloadManager.saveImage(data: data, link: link!)
						self.linkPicture = link!
					} else {
						ProgressHUD.showError("Network error.")
					}
				})
