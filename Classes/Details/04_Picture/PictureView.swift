...
class PictureView: NYTPhotosViewController, SelectUsersDelegate {
	...
	func actionSave() {
		...
		let photoItem = currentlyDisplayedPhoto as! NYTPhotoItem

		UIImageWriteToSavedPhotosAlbum(photoItem.image!, self, #selector(image(_:didFinishSavingWithError:contextInfo:)), nil)
	}
	...
	func didSelectUsers(users: [DBUser]) {
		...
		let predicate = NSPredicate(format: "objectId == %@", photoItem.objectId)
		let dbmessage = DBMessage.objects(with: predicate).firstObject() as! DBMessage
	...
	func actionShare() {
		...
		let activity = UIActivityViewController(activityItems: [photoItem.image], applicationActivities: nil)
